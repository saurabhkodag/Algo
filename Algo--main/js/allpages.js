...
(function($) {
    ...
    $hamburger_menu.click(function() {
        ...
        if ($body.hasClass('mobile-menu')) {
            $main_nav.find($logo).find('img').attr('src', './images/logo_updated.svg');
        } else if (!$body.hasClass('mobile-menu') && $main_nav.hasClass('transparent-header') && !$main_nav.hasClass('et_fixed_nav')) {
            $transparent_header.find($logo).find('img').attr('src', './images/logo-light_updated.svg');
        }
    });
    ...
    $(window).bind("load scroll", $.throttle(200, function() {
        ...
        if (scroll > scrollPosition & !$main_nav.hasClass(fixed_class)) {
            ...
            $transparent_header.find($logo).find('img').attr('src', './images/logo_updated.svg');
